...
export default function ChatSideNav() {
  ...
  // on work 
  const [activeOptSideBar,setactiveOptSideBar] = useState(0);
  // const [activeOptSideBar,setactiveOptSideBar] = useState({element:null,count:0});
  ...
  const handleSubSideNavONOFF = (event) => {
    if ((chatMessageRef.current && chatMessageRef.current.contains(event.target))) {
      setactiveOptSideBar(count=>{return count+1});
      if(activeOptSideBar === 0){
        chatSubSideNavRef.current.classList.add("hidden");
      }else if(activeOptSideBar > 0){
        console.log("activeOptSideBar",activeOptSideBar);
        chatSubSideNavRef.current.classList.toggle("hidden");
      }
    }
      // (chatCallRef.current && chatCallRef.current.contains(event.target))
  }
  // on work !
  ...
  return (
    <>
      <div className={`${chatSideNavwidth ? "w-40" : "w-14"} z-10 pt-18 h-screen w-14 darkTopNav flex flex-col gap-3 p-4 overflow-hidden`}>
        <NavLink ref={chatMessageRef} to={'/chat/'} className={({ isActive }) =>
      ...
      <div ref={chatSubSideNavRef} className="pt-18 darkSubSideNav hidden">
        <ChatSelectContact></ChatSelectContact>
      </div>
    </>
  )
};
